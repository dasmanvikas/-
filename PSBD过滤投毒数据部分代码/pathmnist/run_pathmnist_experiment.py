...
TRAIN_PT = ROOT / "data" / "processed" / "pathmnist_patch.pt"
TEST_PT = ROOT / "data" / "processed" / "pathmnist_test.pt"
OUT_DIR = ROOT / "results"
CKPT_DIR = ROOT / "checkpoints"

# Computed from official PathMNIST train_images, channel order RGB.
PATHMNIST_MEAN = torch.tensor([0.23778291, 0.23778291, 0.23778291]).view(3, 1, 1)
PATHMNIST_STD = torch.tensor([0.35807264, 0.30890018, 0.35218289]).view(3, 1, 1)
...
class PtImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.normalize:
            x = (x - PATHMNIST_MEAN) / PATHMNIST_STD
...
def preact_resnet50_drop(num_classes=9, dropout_p=0.0):
...
def build_loaders(args):
    train_set = PtImageDataset(TRAIN_PT, normalize=True)
    test_set = PtImageDataset(TEST_PT, normalize=True)
...
def main():
    ...
    model = preact_resnet50_drop(num_classes=9, dropout_p=0.0).to(device)
    if args.skip_train:
