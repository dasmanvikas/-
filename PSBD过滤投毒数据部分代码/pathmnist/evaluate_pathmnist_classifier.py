...
from run_pathmnist_experiment import (
    CKPT_DIR,
    OUT_DIR,
    TEST_PT,
    IndexedSubset,
...
def main():
    ...
    macro_precision = sum(x["precision"] for x in per_class) / num_classes
    macro_recall = sum(x["recall"] for x in per_class) / num_classes
    macro_f1 = sum(x["f1"] for x in per_class) / num_classes
    metrics = {
    ...
    ax.set_ylabel("True")
    ax.set_title(f"ResNet-50 PathMNIST Confusion Matrix\nAccuracy={metrics['accuracy']:.4f}")
